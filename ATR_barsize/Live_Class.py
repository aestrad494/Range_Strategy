...
class Live(IB):
    ...
    def download_data(self, tempo, duration):
        ...
        historical = self.ib.reqHistoricalData(self.contract, endDateTime='', durationStr=duration,
                            barSizeSetting=tempo, whatToShow=pr, useRTH=False, keepUpToDate=True)
        return historical
    ...
    def df_continuous(self, df):
        if self.second == 0:
            bars = self.data_to_df(data=self.data[-5:])
            df = pd.concat([df.iloc[:-2], bars.loc[df.index[-2]:]])
        return df
