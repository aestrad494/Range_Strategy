...
class Backtesting():
    ...
    def get_data(self):
        ...
        print('Loading Data...')
        data = pd.read_csv('D:/futuro/Algorithmics/Data/Futures/5_secs/'+self.symbol+'_5secs.csv', index_col=0, parse_dates=True).dropna()
        self.data = data.loc[self.start:self.end]
        print('Data Loaded!')
    
    def convert_data(self, data, tempo='1Min'):
        open_ = data.open.resample(tempo).first().dropna()
        high = data.high.resample(tempo).max().dropna()
        low = data.low.resample(tempo).min().dropna()
        close = data.close.resample(tempo).last().dropna()
        volume = data.volume.resample(tempo).sum().dropna()
    ...
    def order_send(self, type, lots, price=0, bar=0, sl=0, tp=0, comment=''):
        ...
        if self.amount > self.initial_margin or (self.amount < self.initial_margin and self.position != 0):
            ...
            if self.verbose:
                ...
                print('%s %s| current cash balance %7.2f' % (str(self.date), str(self.hour), self.amount))
        else:
