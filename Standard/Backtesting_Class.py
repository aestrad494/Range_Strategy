...
class Backtesting():
    ...
    def order_send(self, type, lots, price=0, bar=0, sl=0, tp=0, comment=''):
        ...
        if self.amount > self.initial_margin or (self.amount < self.initial_margin and self.position != 0):
            ...
            self.pool_check()
            if self.verbose:
                print('%s %s | %sing %d units at %5.2f in %s' % (str(self.date)[:10], str(self.hour), type, lots, price, self.symbol))
                print('%s %s | current cash balance %7.2f' % (str(self.date)[:10], str(self.hour), self.amount))
        else:
