    ...
    def pending_check(self, order):
        ...
        if len(self.pending) > 0:
            ...
            if price > 0:
                ...
                self.pool_check()
                if self.verbose:
                    print('%s %s | %sING %d units at %5.5f in %s (stop)' % (str(self.date), str(self.hour), order_select.type.iloc[0], order_select.lots.iloc[0], price, self.symbol))
                if self.notification:
                    if self.position != 0:
...
if __name__ == '__main__':
    '''symbol = 'ES'
    live = Live(symbol='ES', temp='1 min', client=100, verbose=False, notification=False)'''
