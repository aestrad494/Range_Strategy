...
class OptimizationRange(BackRange):
    def run_optimizer(self):
        ...
        ##Trailing
        start = 0.2; step = 0.2; stop = 0.8
        trailing_list = np.round(list(np.arange(start,stop+step,step)),2)
        trailing_len = len(trailing_list)

        ##Trailing type
        trail_type_list = ['traditional', 'staggered']
        trail_type_len = len(trail_type_list)

        #Number of iterations----------------
        number_iterations = min_len*target_len*trailing_len*trail_type_len
        combinations = list(product(min_list, target_list, trailing_list, trail_type_list))
        print('=' * 55)
        ...
        for combination in combinations:
            ...
            trailing = float(combination[2])
            trailing_type = str(combination[3])
            ...
            self.run_range_strategy(initial_hour='09:30:00', final_hour='15:45:00', ticks_r=1, trades_per_day=1, minutes=minutes, target=target, trailing=trailing, trailing_type=trailing_type)
            ...
            partial_results = [int(minutes), float(target), float(trailing), str(trailing_type),
        ...
        self.final_results.columns = ['minutes', 'target', 'trailing', 'trailing type', 'net profit', 'max drawdown', 
                                      'profit/dd', 'total trades', 'percent profitable', 'max con loss', 'sharpe ratio']
        ...
        if self.select:
            ...
            if len(selected)>0:
                ...
                trailing = selected['trailing'].iloc[0]
                trailing_type = selected['trailing type'].iloc[0]
                print('=' * 55)
                print('Selected Results:')
                print('minutes: %d | target: %5.2f | trailing: %5.2f | trailing type: %s' % (minutes, target, trailing, trailing_type))
                print('=' * 55)
            else:
                print('=' * 55)
...
if __name__ == '__main__':
    ...
    '''optimization = OptimizationRange(symbol='RTY', start='2019-07-11', end='2019-08-11', amount=10000, verbose=False)
    optimization.run_optimizer()'''